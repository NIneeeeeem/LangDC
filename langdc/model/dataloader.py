...
def _get_rawvideo_dec(video_path, image_processor, video_processor, max_frames=8, frame_resolution=224,
                      video_framerate=1, s=None, e=None, min_frames=8, num_video_frames=8, num_context_images=8, fps=0):
    if s is None:
    ...
    if num_frames > 0:
        ...
        if fps > 0: # VIDEOGPT+ fps=1
            all_pos = list(range(f_start, f_end + 1, t_stride))
        else:
            all_pos = list(np.arange(0, num_frames, num_frames / max_frames).astype(int))
