...
from eval.longvideobench.inference.ddp import *
...
def eval_model(args):
    ...
    dataset = LongVideoBenchDataset(".cache/video_data/LongVideoBench", "lvb_val.json", image_processor,video_processor,max_num_frames=16)
    ...
    for (idx, sample_set, video_frames, context_frames, slice_len) in tqdm(dataloader):
        ...
        cap_input_ids = tokenizer_image_token(cap_prompt, cap_tokenizer, return_tensors='pt').unsqueeze(0).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video-folder", type=str, default=".cache/video_data/VSI-Bench")
    parser.add_argument("--qa_path", type=str, default=".cache/video_data/VSI-Bench/test-00000-of-00001.parquet")
    parser.add_argument("--output-dir", type=str, default="MBZUAI/VideoGPT-plus_Phi3-mini-4k/mvbench_eval")
    # parser.add_argument("--conv-mode", type=str, default="conv_qwen2_cap")
