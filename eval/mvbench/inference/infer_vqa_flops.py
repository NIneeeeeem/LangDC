...
def eval_model(args):
    ...
    for (idx, sample_set, video_frames, context_frames, slice_len) in tqdm(dataloader):
        ...
        cap_input_ids = tokenizer_image_token(cap_prompt, tokenizer, return_tensors='pt').unsqueeze(0) # turn to 2-dimension        input_ids_caps = [cap_input_ids.clone() for _ in range(4)]
        ...
        try:
            ...
            # inputs["images"] = torch.tensor([inputs["token_type_ids"]])
            # inputs["attention_mask"] = torch.tensor([inputs["attention_mask"]])
            inputs["images"] = torch.cat(video_frames, dim=0).half()
            inputs["context_images"] = torch.cat(context_frames, dim=0).half()
            inputs["input_ids_caps"] = input_ids_caps
            inputs["hidden_state_layer"] = args.hidden_state_layer
            ...
            with torch.inference_mode():
                output_ids = model.generate(
                    ...
                    input_ids_caps = input_ids_caps.cuda(),
